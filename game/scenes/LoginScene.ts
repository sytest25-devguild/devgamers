...
export default class LoginScene extends Phaser.Scene {
  ...
  create() {
    ...
    const title = this.add.text(0, -130, "Login", {
      fontSize: "34px",
      color: "#f8fafc",
      fontFamily: "Arial",
      fontStyle: "bold",
    ...
    this.statusText = this.add.text(0, 235, "", {
      fontSize: "16px",
      color: "#86efac",
      fontFamily: "Arial",
    }).setOrigin(0.5);
    ...
    this.loginForm = this.add.dom(centerX, centerY + 35).createFromHTML(loginMarkup);
    const formNode = this.loginForm.node as HTMLFormElement | null;
    const backButton = formNode?.querySelector("#back-home") as HTMLButtonElement | null;
    ...
    this.formSubmitHandler = (event: SubmitEvent) => {
      ...
      this.statusText?.setText(
        username.length > 0 ? `Signed in as ${username}` : "Please enter username and password",
      );
    };
